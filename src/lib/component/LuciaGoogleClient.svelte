<script>
    ...
    import { menuExpanded } from '$lib/aerial/stores/index.js'
     ...
     */
    function maybeTransition(node, options) {
...
<!-- <div> -->
    <div class="menu-item w-full md:max-w-fit flex md:justify-end" in:maybeTransition={{ fn: slide, delay: 250, duration: 250, easing: quintOut, axis: 'y' }} out:maybeTransition={{ fn: slide, duration: 250, easing: quintOut, axis: 'y' }}>
        {#if user}
            <!-- in:maybeTransition|global={{ fn: slide, delay: 600, duration: 300, easing: quintOut, axis: 'x' }} out:maybeTransition|global={{ fn: slide, duration: 300, easing: quintOut, axis: 'x' }} -->
            <div class="w-full md:max-w-fit relative inline-block text-left text-slate-800" in:maybeTransition|local={{ fn: slide, delay: 600, duration: 300, easing: quintOut, axis: 'x' }} out:maybeTransition|local={{ fn: slide, duration: 300, easing: quintOut, axis: 'x' }}>
                <!-- Note: Arbitrary width and height values are needed to match the height of other navigation buttons -->
                <button type="button" on:click|stopPropagation={toggleAvatarDropdown} on:keydown={menuKeyboardListener} class="w-full md:max-w-fit flex justify-center p-1 rounded-md md:rounded-full bg-white text-sm font-semibold text-gray-900 shadow-sm ring-4 ring-inset ring-white outline-none border border-transparent hover:ring-indigo-500 focus:ring-indigo-500" id="menu-button" aria-expanded="true" aria-haspopup="true">
                    <img src={user?.picture} alt={user?.name} referrerpolicy="no-referrer" class="w-[1.565rem] h-[1.565rem] md:rounded-full">
                </button>
                {#if open}
                    <div on:click|stopPropagation={()=>{}} on:keydown={menuKeyboardListener} transition:maybeTransition|local={{ fn: slide, duration: 300, easing: quintOut, axis: 'y' }} class="absolute right-0 z-10 mt-2 whitespace-nowrap origin-top-right rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 px-4 py-3 focus:outline-none space-y-2" role="menu" aria-orientation="vertical" aria-labelledby="menu-button" tabindex="-1">
        ...
        {:else}
            ...
            <!-- <a href="/api/oauth/google/login" in:maybeTransition|global={{ fn: slide, delay: 600, duration: 300, easing: quintOut, axis: 'x' }} out:maybeTransition|global={{ fn: slide, duration: 300, easing: quintOut, axis: 'x' }} class="flex items-center gap-2 overflow-hidden rounded-md border-2 border-indigo-300 px-2 py-1.5 outline-none hover:border-indigo-500 focus:border-indigo-500 focus:text-indigo-500">
            ...
            <button title="Sign in with Google" type="button" on:click={login} in:maybeTransition|local={{ fn: slide, delay: 600, duration: 300, easing: quintOut, axis: 'x' }} out:maybeTransition|local={{ fn: slide, duration: 300, easing: quintOut, axis: 'x' }} class="w-full flex items-center justify-center gap-2 overflow-hidden rounded-md border-2 border-indigo-300 px-2 py-1.5 outline-none hover:border-indigo-500 focus:border-indigo-500 focus:text-indigo-500">
                <svg version="1.1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 48 48" class="w-5 h-5"><g><path fill="#EA4335" d="M24 9.5c3.54 0 6.71 1.22 9.21 3.6l6.85-6.85C35.9 2.38 30.47 0 24 0 14.62 0 6.51 5.38 2.56 13.22l7.98 6.19C12.43 13.72 17.74 9.5 24 9.5z"></path><path fill="#4285F4" d="M46.98 24.55c0-1.57-.15-3.09-.38-4.55H24v9.02h12.94c-.58 2.96-2.26 5.48-4.78 7.18l7.73 6c4.51-4.18 7.09-10.36 7.09-17.65z"></path><path fill="#FBBC05" d="M10.53 28.59c-.48-1.45-.76-2.99-.76-4.59s.27-3.14.76-4.59l-7.98-6.19C.92 16.46 0 20.12 0 24c0 3.88.92 7.54 2.56 10.78l7.97-6.19z"></path><path fill="#34A853" d="M24 48c6.48 0 11.93-2.13 15.89-5.81l-7.73-6c-2.15 1.45-4.92 2.3-8.16 2.3-6.26 0-11.57-4.22-13.47-9.91l-7.98 6.19C6.51 42.62 14.62 48 24 48z"></path><path fill="none" d="M0 0h48v48H0z"></path></g></svg>
                <span class="sr-only lg:not-sr-only leading-none !whitespace-nowrap">Sign in with Google</span>
            </button>
        {/if}
